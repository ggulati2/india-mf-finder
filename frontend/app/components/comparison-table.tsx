...
export function ComparisonTable({ funds, selectedFunds }: ComparisonTableProps) {
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500">Select at least 2 funds to compare</p>
          <p className="text-xs text-slate-400 mt-1">We’ll show simple Annual return, Risk, Steadiness</p>
        </div>
      </div>
    );
  }
  ...
  const rows = [
    ...
    { label: "Annual cost", help: "Expense ratio per year", get: (f:Fund)=> fmtTer(f.expense_ratio), cls: "text-slate-600" },
  ];
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100 dark:border-slate-700">
            <th className="text-left p-3 text-xs font-semibold text-slate-500 uppercase">Metric</th>
            {selectedFundsData.map((fund) => (
              <th key={fund.scheme_id} className="text-right p-3">
                <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 max-w-[120px] truncate block">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.label} className="border-b border-slate-50 dark:border-slate-800">
              <td className="p-3 text-slate-600 dark:text-slate-400 font-medium text-xs" title={r.help}>{r.label}<span className="ml-1 text-slate-300">ⓘ</span></td>
              {selectedFundsData.map((fund) => (
      ...
      <p className="text-xs text-slate-400 mt-2 text-center">Tap ⓘ for plain-English help • Advanced Sharpe/Sortino hidden for simplicity</p>
    </div>
  );
}
